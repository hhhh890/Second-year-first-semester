...
def update_user_info(user_id: str, new_name: str, new_gender: str , new_birth_date_str: str = None) -> dict:
    ...
    try:
        ...
        cursor.execute(sql, (new_name, new_gender, new_birth_date_str, user_id))
        connection.commit()
...
def add_friend(current_user_id: str, friend_user_id: str) -> dict:
    ...
    try:
        ...
        cursor.execute(sql, (current_user_id, friend_user_id))
        connection.commit()
...
def delete_friend(current_user_id: str, friend_id: str) -> dict:
    ...
    try:
        ...
        if cursor.rowcount == 0:
            return {"status": False, "msg": "Delete friend failed: Friendship does not exist."}
...
def find_friend(current_user_id: str, friend_name: str) -> dict:
    ...
    try:
        ...
        return {"status": True, "msg": "Friend found successfully", "data": result}
    except mysql.connector.Error as err:
        return handle_db_error(err, "Find friend")
    finally:
        if 'cursor' in locals():
            cursor.close()
        connection.close()
...
def manage_friend(group_id:int,current_user_id: str, friend_id: str) -> dict:
    ...
    try:
        ...
        sql = "UPDATE Friendship SET group_id = %s WHERE user_id = %s AND friend_id = %s"
        cursor.execute(sql, (group_id, current_user_id, friend_id))
        connection.commit()
        
        if cursor.rowcount == 0:
            return {"status": False, "msg": "Manage friend failed: Friendship does not exist."}
        return {"status": True, "msg": "Friend managed successfully."}
    except mysql.connector.Error as err:
...
def create_group(group_name:str,current_user_id: str) -> dict:
    ...
    try:
        ...
        sql = "INSERT INTO FriendGroup(group_name, user_id) VALUES (%s, %s)"
        cursor.execute(sql, (group_name, current_user_id))
        connection.commit()
        
        return {"status": True, "msg": "Group created successfully."}
    except mysql.connector.Error as err:
        return handle_db_error(err, "Create group")
    finally:
        if 'cursor' in locals():
            cursor.close()
        connection.close()
