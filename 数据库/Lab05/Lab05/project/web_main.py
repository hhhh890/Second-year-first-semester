    ...
    if menu == "🏠 Moments Feed":
        ...
        with col2:
            ...
            if action == "Add Friend":
                ...
                if friends_res['status'] and groups_res['status']:
                    f_list = {f['friend_id']: f['name'] for f in friends_res['data']}
                    g_list = {g['group_id']: g['group_name'] for g in groups_res['data']}
            ...
            with c1:
                with st.expander("📝 Update Content"):
                    ...
                    if st.button("Update", key=f"upd_btn_{mid}"):
                        res = update_moment(mid, current_uid, new_c)  # 1. 接收返回值
                        if res['status']: 
                            st.rerun()                                # 2. 成功才刷新
                        else: 
                            st.error(f"❌ {res['msg']}")              # 3. 失败则显示错误提示框
