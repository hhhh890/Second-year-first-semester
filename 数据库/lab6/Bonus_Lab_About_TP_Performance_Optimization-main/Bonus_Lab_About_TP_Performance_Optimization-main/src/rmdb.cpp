        ...
        if (yyparse() == 0) {
            if (ast::parse_tree != nullptr) {
                try {
                    ...
                    // portal
                    // 【临时禁用 savepoint 重试机制】
                    // 当前多线程并发访问全局元数据（sm_manager_->fhs_/ihs_）缺乏互斥保护，
                    // partial_abort 回滚操作可能破坏其他线程正在使用的元数据指针，
                    // 导致 executor 创建时读取损坏的元数据（metadata corrupted）。
                    // 稳定方案需要为 SmManager 所有元数据访问路径添加 mutex，
                    // 或将元数据改为线程局部存储。此处先禁用重试，回到单次执行模式。
