...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid, int n) {
    ...
    assert(n>=0 && n+get_size()<=get_max_size());
    if(n==0) {
        return;
    }
...
bool IxIndexHandle::get_value(const char *key, std::vector<Rid> *result, Transaction *transaction) {
    ...
    std::pair<IxNodeHandle *, bool> leaf_node = find_leaf_page(key, Operation::FIND, transaction);
    if (leaf_node.first==nullptr) {
        return false;
    }
    IxNodeHandle *leaf = leaf_node.first;
    Rid *rid = nullptr;
...
IxNodeHandle *IxIndexHandle::split(IxNodeHandle *node) {
    ...
    int move_cnt = old_size - left_size;
    new_node->insert_pairs(0, node->get_key(left_size), node->get_rid(left_size), move_cnt);
...
void IxIndexHandle::insert_into_parent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node,
    ...
    parent->set_key(old_pos, old_node->get_key(0));
    Rid new_rid = {.page_no = new_node->get_page_no(), .slot_no = -1};
...
page_id_t IxIndexHandle::insert_entry(const char *key, const Rid &value, Transaction *transaction) {
    ...
    // 提示：记得unpin page；若当前叶子节点是最右叶子节点，则需要更新 file_hdr_.last_leaf；记得处理并发的上锁
    ...
    if (is_empty()) {
        ...
        leaf_header->set_next_leaf(root->get_page_no());
        leaf_header->set_prev_leaf(root->get_page_no());
        ...
        page_id_t page_no = root->get_page_no();
        buffer_pool_manager_->unpin_page(root->get_page_id(), true);
        delete root;
        return page_no;
    }
...
bool IxIndexHandle::coalesce_or_redistribute(IxNodeHandle *node, Transaction *transaction, bool *root_is_latched) {
    ...
    bool should_delete_parent = coalesce(&neighbor, &node, &parent, index, transaction, root_is_latched);
    if (should_delete_parent) {
        coalesce_or_redistribute(parent, transaction, root_is_latched);
    }
    IxNodeHandle *extra_node = (neighbor == original_node) ? node : neighbor;
    buffer_pool_manager_->unpin_page(parent->get_page_id(), true);
    buffer_pool_manager_->unpin_page(extra_node->get_page_id(), true);
    delete parent;
    delete extra_node;
    return true;
}
...
bool IxIndexHandle::coalesce(IxNodeHandle **neighbor_node, IxNodeHandle **node, IxNodeHandle **parent, int index,
    ...
    int right_pos = (*parent)->find_child(right);
    (*parent)->erase_pair(right_pos);
    if ((*parent)->get_size() > 0) {
        (*parent)->set_key((*parent)->find_child(left), left->get_key(0));
        maintain_parent(*parent);
    }

    return (*parent)->is_root_page() || (*parent)->get_size() < (*parent)->get_min_size();
