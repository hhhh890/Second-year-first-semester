...
/**
 * @description: 部分回滚事务到 savepoint
 * @param {Transaction*} txn 需要部分回滚的事务
 *
 * 实现说明：
 *   倒序回滚 write_set 中 savepoint 之后的写操作（与 abort 的回滚逻辑相同），
 *   但不释放锁、不改变事务状态、不从全局事务表移除、不 delete txn。
 *   回滚后事务保持 GROWING 状态，可继续执行（重试当前语句）。
 *   用于 wait-die 死锁预防触发 abort 时，避免回滚整个事务。
 */
void TransactionManager::partial_abort(Transaction* txn) {
    if (txn == nullptr) return;

    auto write_set = txn->get_write_set();
    size_t savepoint = txn->get_savepoint();

    // 倒序回滚 savepoint 之后的写操作
    while (write_set->size() > savepoint) {
        WriteRecord *wr = write_set->back();
        write_set->pop_back();
        const std::string &tab_name = wr->GetTableName();
        const Rid &rid = wr->GetRid();
    ...
    // 不释放锁、不改变状态、不从 txn_map 移除 —— 事务保持活跃可继续执行
}