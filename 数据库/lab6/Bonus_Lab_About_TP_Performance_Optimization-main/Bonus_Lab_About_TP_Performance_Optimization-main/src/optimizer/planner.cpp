...
// 索引匹配规则（Changelog 第5节）：
//   1. 等值左前缀最长优先：遍历所有索引，找到 WHERE 条件中匹配最多连续等值列的索引
//   2. 等值前缀后的第一列如果是范围条件（< <= > >=），也允许选择该索引
//   3. 单列索引上的范围条件也允许使用索引（如 id > 4）
//   4. 不跳过左前缀：WHERE a=1 AND c=3（索引是 a,b,c）→ 只匹配 a 等值前缀，不使用 c
bool Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds, std::vector<std::string>& index_col_names) {
    ...
    // 构建条件映射：col_name -> Condition（只处理 lhs_col.tab_name == tab_name 且 is_rhs_val）
    ...
    // 遍历所有索引，找到匹配度最高的
    int best_score = -1;  // 等值前缀长度 * 10 + (是否有范围条件)
    std::vector<std::string> best_index_cols;

    for(auto& index : tab.indexes) {
        int eq_prefix_len = 0;
        bool range_after = false;
        int score = 0;
        ...
        for(size_t i = 0; i < index.col_num; ++i) {
            ...
            Condition& cond = it->second;
            if(cond.op == OP_EQ) {
                eq_prefix_len++;
            } else if((cond.op == OP_LT || cond.op == OP_LE || cond.op == OP_GT || cond.op == OP_GE) && i == eq_prefix_len) {
                // 等值前缀后的第一列是范围条件（包括 eq_prefix_len==0 的情况）
                range_after = true;
                break;
            } else {
                // 非 EQ 且非有效范围位置 → 前缀结束
        ...
        // 计算分数：等值前缀长度 * 10 + 范围条件权重（1）
        score = eq_prefix_len * 10 + (range_after ? 1 : 0);
...
std::shared_ptr<Plan> Planner::make_one_rel(std::shared_ptr<Query> query)
{
    ...
    for (size_t i = 0; i < tables.size(); i++) {
        ...
        // 【调试】验证索引选择是否生效
        if (index_exist) {
            std::cerr << "[DEBUG] SELECT on " << tables[i] << " uses index: ";
            for (auto& col : index_col_names) std::cerr << col << " ";
            std::cerr << " (conds: " << curr_conds.size() << ")" << std::endl;
        } else {
            std::cerr << "[DEBUG] SELECT on " << tables[i] << " uses SeqScan (no index matched, conds: " << curr_conds.size() << ")" << std::endl;
        }
...
std::shared_ptr<Plan> Planner::do_planner(std::shared_ptr<Query> query, Context *context)
{
    ...
    if (auto x = std::dynamic_pointer_cast<ast::CreateTable>(query->parse)) {
        ...
        // 【调试】验证 DELETE 索引选择
        if (index_exist) {
            std::cerr << "[DEBUG] DELETE on " << x->tab_name << " uses index: ";
            for (auto& col : index_col_names) std::cerr << col << " ";
            std::cerr << " (conds: " << query->conds.size() << ", X lock)" << std::endl;
        } else {
            std::cerr << "[DEBUG] DELETE on " << x->tab_name << " uses SeqScan (no index matched, conds: " << query->conds.size() << ", X lock)" << std::endl;
        }

        if (index_exist == false) {  // 该表没有索引
            index_col_names.clear();
            table_scan_executors =
                std::make_shared<ScanPlan>(T_SeqScan, sm_manager_, x->tab_name, query->conds, index_col_names, true);  // DELETE 提前拿 X 锁
        } else {  // 存在索引
            table_scan_executors =
                std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, x->tab_name, query->conds, index_col_names, true);  // DELETE 提前拿 X 锁
        }
        ...
        // 【调试】验证 UPDATE 索引选择
        if (index_exist) {
            std::cerr << "[DEBUG] UPDATE on " << x->tab_name << " uses index: ";
            for (auto& col : index_col_names) std::cerr << col << " ";
            std::cerr << " (conds: " << query->conds.size() << ", X lock)" << std::endl;
        } else {
            std::cerr << "[DEBUG] UPDATE on " << x->tab_name << " uses SeqScan (no index matched, conds: " << query->conds.size() << ", X lock)" << std::endl;
        }

        if (index_exist == false) {  // 该表没有索引
        index_col_names.clear();
            table_scan_executors =
                std::make_shared<ScanPlan>(T_SeqScan, sm_manager_, x->tab_name, query->conds, index_col_names, true);  // UPDATE 提前拿 X 锁
        } else {  // 存在索引
            table_scan_executors =
                std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, x->tab_name, query->conds, index_col_names, true);  // UPDATE 提前拿 X 锁
        }
