...
{
    public:
        ScanPlan(PlanTag tag, SmManager *sm_manager, std::string tab_name, std::vector<Condition> conds, std::vector<std::string> index_col_names, bool exclusive_lock = false)
        {
            Plan::tag = tag;
        ...
        bool exclusive_lock_;  // 新增：是否在扫描阶段申请 X 锁
};
