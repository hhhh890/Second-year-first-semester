...
class SeqScanExecutor : public AbstractExecutor {
   private:
    ...
    bool exclusive_lock_;  // 新增：写扫描提前拿 X 锁

   public:
    SeqScanExecutor(SmManager *sm_manager, std::string tab_name, std::vector<Condition> conds, Context *context, bool exclusive_lock = false) {
        ...
        context_ = context;
        exclusive_lock_ = exclusive_lock;  // 新增：写扫描提前拿 X 锁
    ...
    void beginTuple() override {
        // 提前 X 锁优化：UPDATE/DELETE 的定位扫描直接拿 X 锁，避免锁升级冲突
