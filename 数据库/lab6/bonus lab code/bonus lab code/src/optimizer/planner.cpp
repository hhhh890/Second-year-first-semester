...
// 选择可用索引：等值前缀优先；等值前缀后的第一列如果有范围条件，也可以使用索引。
// IndexScan 会把这些条件拼成 lower/upper key，交给 B+ 树做有序范围扫描。
bool Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds, std::vector<std::string>& index_col_names) {
    index_col_names.clear();
    TabMeta& tab = sm_manager_->db_.get_table(tab_name);
    std::set<std::string> eq_cols;
    std::set<std::string> range_cols;
    ...
    for (auto &index : tab.indexes) {
        ...
        for (auto &col : index.cols) {
            if (eq_cols.count(col.name) != 0) {
                ++eq_prefix;
                continue;
            }
            if (range_cols.count(col.name) != 0) {
                has_range = true;
                break;
            }
            break;
        }
        if (eq_prefix == 0 && !has_range) {
            continue;
        }
        int score = static_cast<int>(eq_prefix) * 2 + (has_range ? 1 : 0);
        if (score > best_score) {
            best_score = score;
            best_cols.clear();
            for (auto &col : index.cols) {
    ...
    if (auto x = std::dynamic_pointer_cast<ast::CreateTable>(query->parse)) {
        ...
        if (index_exist == false) {  // 该表没有索引
            ...
            table_scan_executors = 
                std::make_shared<ScanPlan>(T_SeqScan, sm_manager_, x->tab_name, query->conds, index_col_names, true);
        } else {  // 存在索引
            table_scan_executors =
                std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, x->tab_name, query->conds, index_col_names, true);
        }
