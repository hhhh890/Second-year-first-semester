...
class IndexScanExecutor : public AbstractExecutor {
   ...
   public:
    IndexScanExecutor(SmManager *sm_manager, std::string tab_name, std::vector<Condition> conds, std::vector<std::string> index_col_names,
                    Context *context, bool exclusive_lock = false) {
        sm_manager_ = sm_manager;
        context_ = context;
        exclusive_lock_ = exclusive_lock;
    ...
    void beginTuple() override {
        ...
        auto fill_min_key = [](char *dst, const ColMeta &col) {
            if (col.type == TYPE_INT) {
                ...
                memset(dst, 0x00, col.len);
            }
        };
        auto fill_max_key = [](char *dst, const ColMeta &col) {
            if (col.type == TYPE_INT) {
        ...
        for (size_t i = 0; i < index_col_names_.size(); ++i) {
            ...
            if (it_lo != lower_conds.end()) {
                ...
                fill_min_key(lower_key.data() + prefix_eq_offset, col);
            }
            if (it_up != upper_conds.end()) {
                memcpy(upper_key.data() + prefix_eq_offset, it_up->second->rhs_val.raw->data, col.len);
                use_index_lookup = true;
            } else {
                fill_max_key(upper_key.data() + prefix_eq_offset, col);
            }
            // 剩余列填充边界
            int rest_off = prefix_eq_offset + col.len;
            for (size_t j = i + 1; j < index_col_names_.size(); ++j) {
                fill_min_key(lower_key.data() + rest_off, index_meta_.cols[j]);
                fill_max_key(upper_key.data() + rest_off, index_meta_.cols[j]);
